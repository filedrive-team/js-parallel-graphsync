...
const sniffSignatures = [
    ...
    new htmlSig("<!--"),
    new svgSig(),
    new maskedSig(Buffer.from([0xff, 0xff, 0xff, 0xff, 0xff]), Buffer.from("<?xml"), true, "text/xml; charset=utf-8"),
    new exactSig(Buffer.from("%PDF-"), "application/pdf"),
    new exactSig(Buffer.from("%!PS-Adobe-"), "application/postscript"),
    // UTF BOMs.
    new maskedSig(Buffer.from([0xff, 0xff, 0x00, 0x00]), Buffer.from([0xfe, 0xff, 0x00, 0x00]), false, "text/plain; charset=utf-16be"),
    new maskedSig(Buffer.from([0xff, 0xff, 0x00, 0x00]), Buffer.from([0xff, 0xfe, 0x00, 0x00]), false, "text/plain; charset=utf-16le"),
    new maskedSig(Buffer.from([0xff, 0xff, 0xff, 0x00]), Buffer.from([0xef, 0xbb, 0xbf, 0x00]), false, "text/plain; charset=utf-8"),
    new exactSig(Buffer.from("GIF87a"), "image/gif"),
    new exactSig(Buffer.from("GIF89a"), "image/gif"),
    new exactSig(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]), "image/png"),
    new exactSig(Buffer.from([0xff, 0xd8, 0xff]), "image/jpeg"),
    new exactSig(Buffer.from("BM"), "image/bmp"),
    new maskedSig(Buffer.from([
        0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff,
    ]), Buffer.from("RIFF\x00\x00\x00\x00WEBPVP"), false, "image/webp"),
    new exactSig(Buffer.from([0x00, 0x00, 0x01, 0x00]), "image/vnd.microsoft.icon"),
    new maskedSig(Buffer.from([
        0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
    ]), Buffer.from("RIFF\x00\x00\x00\x00WAVE"), false, "audio/wave"),
    new maskedSig(Buffer.from([
        0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
    ]), Buffer.from("FORM\x00\x00\x00\x00AIFF"), false, "audio/aiff"),
    new maskedSig(Buffer.from([0xff, 0xff, 0xff, 0xff]), Buffer.from(".snd"), false, "audio/basic"),
    new maskedSig(Buffer.from([0xff, 0xff, 0xff, 0xff, 0xff]), Buffer.from("OggS\x00"), false, "application/ogg"),
    new maskedSig(Buffer.from([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]), Buffer.concat([Buffer.from("MThd"), Buffer.from([0x00, 0x00, 0x00, 0x06])]), false, "audio/midi"),
    new maskedSig(Buffer.from([0xff, 0xff, 0xff]), Buffer.from("ID3"), false, "audio/mpeg"),
    new maskedSig(Buffer.from([
        0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
    ]), Buffer.from("RIFF\x00\x00\x00\x00AVI "), false, "video/avi"),
    new exactSig(Buffer.from([0x1a, 0x45, 0xdf, 0xa3]), "video/webm"),
    new exactSig(Buffer.from([0x52, 0x61, 0x72, 0x20, 0x1a, 0x07, 0x00]), "application/x-rar-compressed"),
