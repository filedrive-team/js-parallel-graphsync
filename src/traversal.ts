  ...
  return {
    ...
    parseSelector(node: SelectorNode): Selector {
      ...
      switch (key) {
        ...
        case "|":
          return this.ParseExploreUnion(node[key])
        default:
    ...
    ParseExploreUnion(node: any): Selector{
      const x :Selector[] = [];
      node.forEach((m:any)=>{
        const member= this.parseSelector(m)
        x.push(member)
      })
      return new ExploreUnion(x);
    },
...
export class ExploreUnion implements Selector{
  ...
  explore(node: any, path: PathSegment): Selector | null {
    ...
    if(nonNilResults.length === 0){
      return null;
    }
    if(nonNilResults.length === 1){
      return nonNilResults[0];
    }
    return new ExploreUnion(nonNilResults);
  }
  decide(node: any): boolean {
    let _bool = false
    this._members.forEach((m)=>{
